...
public class bl_Joystick : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    [SerializeField, Range(0.5f, 4)] private float OnPressScale = 1.2f;//return to default position speed
    ...
    {
        //Debug.Log( "x = " + (StickRect.position.x -DeathArea.x) /Radio);
        //Debug.Log( "y = " + (StickRect.position.y - DeathArea.y) /Radio);
        if((StickRect.position.x - DeathArea.x) / Radio > 0 )
        {
        ...
        if((StickRect.position.y - DeathArea.y )/ radio > 0)
        {
            theRotation.TurnUp();
        }
        if((StickRect.position.y - DeathArea.y) / Radio < 0)
